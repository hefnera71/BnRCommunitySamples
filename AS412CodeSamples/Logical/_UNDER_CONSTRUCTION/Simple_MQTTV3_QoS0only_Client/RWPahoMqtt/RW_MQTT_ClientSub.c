// Copyright (c) 2025 Alexander Hefner, Steffen Herderich
// 
// Licensed under the MIT License - see LICENSE.txt file for details

/*
Information about the code under the package (= folder) "MQTTPacket" 

The code used inside the package "MQTTPacket" was gathered from the paho.mqtt.embedded-c repository at Github
https://github.com/eclipse-paho/paho.mqtt.embedded-c/tree/master/MQTTPacket/src

This code out of the PahoMQTT and the accompanying materials are made available under the terms of the Eclipse Public License v1.0
and Eclipse Distribution License v1.0 which accompany this distribution.

Both licence texts can be found here in this package:
MQTTPacket/edl-v10.txt
MQTTPacket/epl-v10.txt
*/
...
#include <RWPahoMqtt.h>
#include <AsBrStr.h>
USINT tcpBuffer[rwmqtt_INTERNAL_BUFFER_SIZE];
...
void RW_MQTT_ClientSub(struct RW_MQTT_ClientSub* inst)
{
	...
	switch(inst->uiState) 
	{
		case 0: 
			if (inst->I_xEnable == 1){
				inst->uiState = 10;
				brsmemset(&tcpBuffer, 0, sizeof(tcpBuffer));
			}
			//inst->I_xSubscribe = 0;
			break;
		...
		case 30:
			...
			inst->TcpSend_0.datalen = inst->Q_iPahoFcCallResult = MQTTSerialize_connect(&tcpBuffer, sizeof(tcpBuffer), &MQTTPacket_connectData_);
		...
		case 31:
			...
			inst->TcpSend_0.pData = &tcpBuffer;
		...
		case 32:
			...
			inst->TcpRecv_0.pData = &tcpBuffer;
			inst->TcpRecv_0.datamax = sizeof(tcpBuffer);
